			...
			<div class="left-panel">
				<div class="panel">
					...
					<div class="content marbottom30">
						<h3>
							<form id="transactionhistory" method="post"
								action="TransactionServlet">
								<ul>
								 <c:if test="${LoginPerson =='ADMIN'}">
								 <li id="li_5"><label>Account Number </label> <select
										name="ActNum">
											<option value="Select Account number" selected="selected">select you
												account</option>
											<c:forEach items="${AllAccounts}" var="accounts">
												<option value="${accounts.getAccountId()}">${accounts.getAccountId()}</option>
											</c:forEach>
									</select></li>
									<li id="li_5"><label>Action </label> <select
										name="ActAction">
											<option value="Select Action" selected="selected">select your
												Action</option>
											<option value="Last 10 Transactions">Last 10 Transactions</option>
											<option value="All Transactions">All Transactions</option>
											<option value="Transaction between time period">Transaction between time period</option>
									</select></li>
								 </c:if>
									...
									<li id="li_5"><label>Account Number </label> <select
										name="ActNum">
											...
											<c:forEach items="${users}" var="user">
												<option value="${user.getAccountId()}">${user.getAccountId()}</option>
											</c:forEach>
									</select></li>
									<li id="li_5"><label>Action </label> <select
										name="ActAction">
